...
Publication rechecks certificates, access policy, Compose fingerprints and existing hosts before applying. Shared networking reuses an unambiguous persistent service alias, or adds both services to an assistant-managed internal bridge specific to that NPM instance; original networks are retained. Adding a network can restart both services. Explicit `network_mode` must be converted in app settings first. The upstream uses HTTP and an observed internal TCP port; the published host uses HTTPS, HTTP-to-HTTPS redirection and WebSockets.
