...
- **Inspect only:** observation tools, public web search and page reads can run; downloads and other write tools cannot execute.
...
- **Automatic changes:** the tools may install, configure and restart apps, and download supplemental app data for the task without individual approvals. Content and destination restrictions still apply.
...
| `web_search` | Searches the public web and returns up to eight source titles/URLs. No additional API key is required. Reports provider challenges/failures explicitly. |
| `web_read` | Reads up to 1 MiB of public HTML, text, XML or JSON; returns a bounded text excerpt and public links. Supports text offsets. No JavaScript or login session. |
| `app_content` | Reports an installed service and its eligible persistent content mounts. The LLM decides what content belongs in the app from its purpose, user request and documentation. |
| `download_app_content` | Downloads app-related data into an eligible content directory, creating missing subdirectories. Checks for executable content and unsafe archives before publishing a new file; reports bytes and SHA-256. Never overwrites. Existing permission modes govern execution. |

## Public web and supplemental app content

This is a general capability, independent of any content site. A typical request is “Find this book and put the MOBI in my Calibre library” or “Find this homebrew game and download its ROM for my emulator.” The LLM follows the embedded [supplemental content instruction](../internal/assistant/instructions/app-content.md): inspect the installed app and its storage, determine why the file is relevant and compatible, establish the documented import location, search/read sources, and propose a direct-file download. Its free-text `purpose` explains the file, its use in this app and the destination in the approval summary. Web content is untrusted evidence; embedded requests cannot authorize further actions. Search queries and page URLs are sent to the requested public services; provider/app credentials are not attached.

Search uses [DuckDuckGo's non-JavaScript HTML search](https://duckduckgo.com/duckduckgo-help-pages/features/non-javascript), with [Bing RSS search](https://blogs.bing.com/search/January-2005/RSS-Feeds-for-Search-Results) as an independent fallback. It needs no new account. The result identifies the source and whether fallback was used. If both providers fail or return a challenge/unsupported layout, the tool reports an error rather than an empty successful search. Users can also supply a public direct URL. Sites requiring JavaScript, authentication or interactive challenges are not supported by these tools.

Content suitability is an LLM judgment, not an algorithmic classification. There is no app registry, app-to-format table, category enum or allowlist of data formats. Calibre/MOBI and emulator/ROM are examples in the instruction, not special cases in Go. New kinds of supplemental data (such as dictionaries, datasets or unfamiliar application formats) need no backend registration. The LLM must establish relevance to the user's request and compatibility with the actual app, consult documentation as needed and ask when uncertain. Passing server safety checks is not proof of relevance or compatibility.

The backend retains a uniform technical no-execution boundary: it rejects executable/script/installer filenames, known host binary signatures (including PE, ELF, Mach-O, Java, WebAssembly) and common script headers. ZIP containers are identified by their bytes independently of filenames, so ZIP-based data formats are inspected without categorizing the app. Every entry is checked for executable names/headers, nested archives, traversal, duplicate names, links and encryption; markup is checked for scripts, event handlers and embedded programs. Decompression is limited to the approved byte limit and 512 entries. Opaque containers (RAR, 7z, tar, etc.) and disc images cannot currently be inspected and are refused. Checks do not validate a file against an app-specific format or scan it with antivirus. Unknown ordinary data extensions are allowed; relevance remains the model's responsibility.

Destinations are container directories resolved through the selected service's most specific writable bind mount, with an existing host source below `/DATA`. Named volumes, read-only mounts and system/config destinations are rejected. When an app only has a config mount, add a dedicated content mount (for example `/books`) using `configure_service` first. Directory descriptors opened without following symlinks prevent path swaps from redirecting writes. Missing content subdirectories inherit the parent owner when running as root. New files use mode 0644. The assistant holds the shared app-operation lock and rejects changed service settings after approval.

The proposal names the app, service, purpose, source URL, destination and maximum byte count. Each download is limited to 4 GiB and 15 minutes, within the existing run limit. The chat displays live byte progress; **Stop task** cancels the transfer. Downloads stay in a private staging directory on the destination filesystem until safety checks complete; a same-filesystem hard link publishes the completed file without replacing any existing entry. Failed/cancelled transfers remove their staging files; directories created for the destination may remain. A process crash can leave a private `.casaos-download-*` staging directory and an uncertain outcome, which history recovery reports without replaying the download. There is no background queue or resume across restarts.

Public web connections use standard HTTP(S) ports, validate DNS results and each redirect, dial the validated address, refuse HTTPS downgrades, and disable environment proxies and cookies. Loopback, private, link-local and reserved destinations are blocked, including through DNS and redirects. These tools cannot reach local app administration endpoints; existing app-specific tools retain that role.

The result reports the saved container path, size and SHA-256, and verifies an optional user/source checksum. Saving a file does not imply it was imported into Calibre, indexed by an emulator or added to a media library. The assistant must verify the app's documented scan/import workflow or explain the remaining step. Inspectable archives are checked and saved intact, not automatically extracted. No downloaded content is executed.

Validation (ordinary tests use local fixtures, not public content downloads):

```sh
go test ./internal/assistant ./service -run 'TestPublicWeb|TestWebBody|TestToolProgress|TestAssistant(Web|Content|Download)' -count=1
CASAOS_ASSISTANT_WEB_LIVE_TEST=1 go test ./service -run '^TestAssistantWebLive$' -count=1 -v
```
