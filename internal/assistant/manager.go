...
func (m *Manager) run(ctx context.Context, s *session) {
	...
	for {
		...
		if len(s.queue) > 0 {
			...
			s.mu.Unlock()
			output, err := prepared.Execute(toolProgressContext(ctx, s, call.Function.Name))
			s.mu.Lock()
			if s.view.Status != "running" {
				finish()
				return
